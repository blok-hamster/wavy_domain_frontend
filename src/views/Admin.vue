    ...
    <div class="row mt-3 text-center">
      <div class="col-md-8 offset-md-2">
        ...
        <div v-if="isUserMinterAdmin">
          ...
          <p>
            Important: Referral fees are defined in basis points (bps). Example:
            1000 bps = 10%
          </p>
        ...
        <div v-if="isUserTldAdmin">
          <h3>TLD contract (.getwavy): transfer ownership</h3>
...
export default {
  ...
  methods: {
    ...
    async changePrice(chars) {
      ...
      try {
        const tx = await minterContractSigner.changePrice(
          newPriceWei,
          Number(chars)
        );
    ...
    async changeReferralFee() {
      ...
      try {
        const tx = await minterContractSigner.changeReferralFee(
          Number(this.newReferralFee)
        );
    ...
    async transferMinterOwnership() {
      ...
      try {
        const tx = await minterContractSigner.transferOwnership(
          this.newMinterOwner
        );
    ...
    async transferTldOwnership() {
      ...
      const tldContractSigner = new ethers.Contract(
        this.getTldAddress,
        tldIntfc,
        this.signer
      );
